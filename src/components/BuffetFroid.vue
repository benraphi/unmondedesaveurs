<style scoped>
.imgfond{
  margin:0;
  padding:0;
  background: url("../../static/img/pages/Buffet.jpg") no-repeat center fixed;
  background-size:cover;
  width:100vw;
  height: 50vh;
  font-size: 5vw;
  font-weight: bold;
  font-family: 'Times New Roman';
  line-height: 50vh;
  color:black;
  margin-bottom: 0.3em;
}
h2{
    margin-bottom: 1em;
}
h3{
  text-decoration: underline;
}
</style>
<template>
  <div>
    <div class="imgfond">
      <p>Buffet froid</p>
    </div>
    <h3>Buffet à 25€ :</h3>
    <p>
      3 mises en bouche<br>
      2 choix de crudités<br>
      2 choix de salades composées<br>
      2 choix d'entrées<br>
      2 choix de viandes/poissons<br>
      Buffet de desserts : 3 desserts<br>
    </p>
    <h3>Buffet à 30€ :</h3>
    <p>
      4 mises en bouche<br>
      3 choix de crudités<br>
      2 choix de salades composées<br>
      3 choix d'entrées<br>
      3 choix de viandes/poissons<br>
      Buffet de desserts : 3 desserts<br>
    </p>
    <h3>Buffet à 35€ :</h3>
    <p>
      5 mises en bouche<br>
      3 choix de crudités<br>
      3 choix de salades composées<br>
      3 choix d'entrées<br>
      4 choix de viandes/poissons<br>
      plateau de fromages (5 sortes)<br>
      Buffet de desserts : 4 desserts<br>
    </p>
    <h2><u>Choix des éléments du buffet :</u></h2>
    <h2>Mises en bouche :</h2>
    <p>
      Verrine de foie gras, confiture d'oignons, dés de pain d'épice.<br>
      Verrine de cappuccino d'avocat et crème de xérès.<br>
      Verrine de gaspacho de tomates et crevettes parfumées à la coriandre.<br>
      Mini poêlon de panacota de chèvre et tartare de tomates aux olives.<br>
      Mini poêlon de panacotta de poivron doux et pétoncles poêlés aux pignons de pin.<br>
      Cuillère garnie de granité d'ananas et gingembre, poulet grillé.<br>
      Cuillère garnie de tartare aux deux saumons et pomme verte.<br>
      Muffin poitrine fumée au thym.<br>
      Muffins chorizo et deux poivrons.<br>
      Mini brochettes tomates confites, mozarella et jambon cru.<br>
      Mini brochettes pomme, gouda et poulet paprika.<br>
    </p>
    <h2>Crudités en saladier :</h2>
    <p>
      Concombre au fromage blanc et menthe.<br>
      Carottes râpées, brunoise de pommes et vinaigrette au citron, olive.<br>
      Betterave à la vinaigrette de noisettes.<br>
      Taboulé à ma façon.<br>
      Salade de tomates cocktails et billes de mozzarella, vinaigrette olive balsamique.<br>
      Salade de pommes de terre à l'estragon et poitrine fumée.<br>
    </p>
    <h2>Salades composées en saladier :</h2>
    <p>
      Salade de pennes au gambas et pétoncles et petits légumes croquants, vinaigrette au balsamique.<br>
      Céleri rémoulade au curry, crevettes et dés de poulet.<br>
      Salade de légumes verts et magret fumé, vinaigrette à la framboise.<br>
      Salade de Coleslaw à l'Emmental.<br>
    </p>
    <h2>Entrées :</h2>
    <p>
      Saumon fumé par mes soins.<br>
      Terrine de lapin aux pruneaux.<br>
      Terrine de canard aux pistaches.<br>
      Terrine de foie de volailles au porto.<br>
      Terrine de rillettes aux deux saumons.<br>
      Tomates au thon sauce cocktail.<br>
      Tomates garnies aux deux crevettes.<br>
    </p>
    <h2>Viandes et poissons :</h2>
    <p>
      Assortiment de charcuterie (jambon blanc et cru, roulade, salami, rosette).<br>
      Potjewleech (assortiment de quatre viandes en gelée).<br>
      Filet de dinde parfumé aux fines herbes.<br>
      Filet de rôti de porc au paprika.<br>
      Rôti de boeuf en croûte de poivre.<br>
      Pilons de poulet au miel et soja.<br>
      Tartare de cabillaud aux fins condiments.<br>
      Pavé de saumon bellevue.<br>
      Potjewleech de poissons (assortiment de poissons en gelée).<br>
    </p>
    <h2>Buffet de desserts :</h2>
    <p>
      <u>dont un gâteau avec décor et inscription :<br></u>
      Croustillant aux trois chocolats.<br>
      Moelleux aux deux chocolats (biscuit moelleux chocolat et noix, mousse chocolat blanc et chocolat noir).<br>
      Charlotte poire/chocolat ou fruits rouges.<br>
      Bavarois fruits rouges ou poire/caramel ou framboise/passion.<br>
      Tutti frutti (génoise, crème mousseline et fruits).<br>
      Fraisier ou framboisier (sauf hors saison).<br>
      Tarte pomme/cannelle ou tarte poire/frangipane ou tarte citron.<br>
      Progrès café ou chocolat ou praliné.<br>
      Gâteau glacé. Parfum : vanille/framboise ou vanille/fraise ou vanille/chicorée ou nougat ou passion/framboise.<br>
      Assortiment de choux café, chocolat et vanille.<br>
      Mousseux irlandais : biscuit aux noix, mousse café/whisky, mousse chocolat blanc.<br>
      Pyramide de profiteroles, glace spéculoos, sauce chocolat blanc.<br>
      Pyramide de profiteroles, glace vanille, sauce chocolat noir.<br>
      <u>Assortiment de verrines sucrées :<br></u>
      Verrine de confit d'ananas, crème de mascarpone à la vanille et broyé de spéculoos.<br>
      Verrine de mousse chocolat noir, craquant au chocolat blanc et griottes.<br>
      Verrine de mousse chocolat blanc, craquant au chocolat noir et griottes.<br>
      Verrine de minestrone de fruits au sirop épicé.<br>
      Verrine de mousse de fromage blanc, coulis de fruits rouges, groseilles.<br>
      Verrine de Tiramisu.<br>
      Verrine de chti'ramisu (genièvre et spéculoos).<br>
      Verrine de mousseline de pain d'épice et pommes confites au miel.<br>
      Verrine de panacotta à la noix de coco et citron vert, coulis de mangue.<br>
      Verrine de carambar et compotée de pommes.<br>
    </p>
  </div>
</template>
