<style scoped>
.imgfond{
  margin:0;
  padding:0;
  background: url("../../static/img/pages/Barbecue.jpg") no-repeat center fixed;
  background-size:cover;
  width:100vw;
  height: 50vh;
  font-size: 5vw;
  font-weight: bold;
  font-family: 'Times New Roman';
  line-height: 50vh;
  color:black;
  margin-bottom: 0.3em;
}
h2{
    margin-bottom: 1em;
}
</style>
<template>
  <div>
    <div class="imgfond">
      <p>Barbecue party</p>
    </div>
    <h2>35€ par personne.<br>
      Minimum 15 personnes. Prestation sur place et prêt de barbecue inclus.<br>
    </h2>
    <h2>Mises en bouche (5 mises en bouche par personne) :</h2>
    <p>
      Verrine de cappuccino d'avocat, crème de xérès.<br>
      Verrine de gaspacho de tomates et crevettes parfumées à la coriandre.<br>
      Verrine de foie gras, confits d'oignons aux raisins, dés de pain d'épice.<br>
      Mini poêlon de panacotta de chèvre et tartare de tomates aux olives.<br>
      Mini poêlon de panacotta de poivron doux et pétoncles poêlées aux pignons de pins.<br>
      Cuillère garnie tartare aux deux saumons, pomme verte.<br>
      Cuillère garnie granité d'ananas et gingembre, poulet grillé.<br>
      Muffin poitrine fumée et thym.<br>
      Muffin chorizo et deux poivrons.<br>
      Mini brochette tomates confites, mozzarella et jambon cru.<br>
      Mini brochette pommes, gouda et poulet paprika.<br>
    </p>
    <h2>Crudités en saladier (3 choix possible) :</h2>
    <p>
      Concombre au fromage blanc et menthe.<br>
      Carottes râpées, brunoise de pommes et vinaigrette citron, olive.<br>
      Betterave à la vinaigrette de noisettes.<br>
      Taboulé à ma façon.<br>
      Macédoine maison.<br>
      Céleri rémoulade au curry.<br>
    </p>
    <h2>Salades composées en saladier (3 choix possible) :</h2>
    <p>
      Salade de pennes aux petits légumes croquants, vinaigrette au basilic.<br>
      Salade de légumes verts, vinaigrette à la framboise.<br>
      Salade de tomates coktails et billes de mozzarella, vinaigrette olive balsamique.<br>
      Salade de pommes de terre à l'estragon et poitrine fumée.<br>
      Salade Coleslaw à l'Emmental.<br>
    </p>
    <h2>Grillades de viandes :</h2>
    <p>
      Petit tournedos de filet de boeuf et lard.<br>
      Brochette de porc au chorizo et oignon rouge.<br>
      Brochette de dinde marinée aux épices.<br>
      Brochette d'agneau à l'ail et fines herbes.<br>
    </p>
    <h2>Grillades de poissons :</h2>
    <p>
      Petit tournedos de saumon et courgettes.<br>
      Brochette de Saint-Jacques et citron vert.<br>
      Brochette de thon aux graines de sésame.<br>
      Brochette de gambas et légumes.<br>
    <h2>Fromage : (3€ supplémentaire par personne)</h2>
    <p>Camembert.<br>
    Maroilles.<br>
    </p>
    <h2>Dessert :</h2>
    <p>
      <u>
      Gâteau de votre choix :<br>
      </u>
      Croustillant aux trois chocolats<br>
      Fraisier<br>
      Framboisier<br>
      Moelleux aux deux chocolats (biscuit moelleux chocolat et noix, mousse chocolat blanc et chocolat noir).<br>
      Charlotte poire/chocolat ou fruits rouges.<br>
      Tutti frutti (génoise, crème mousseline et fruits).<br>
      Progrès café ou chocolat ou praliné<br>
      Gâteau glacé. Parfum vanille/framboise ou vanille/fraise ou vanille/chicorée ou nougat ou passion/framboise.<br>
      Assortiment de choux chocolat, café et vanille.<br>
      Mousseux Irlandais : biscuit aux noix, mousse café/whisky, mousse chocolat blanc.<br>
      Pyramide de profiteroles, glace vanille, sauce chocolat noir.<br>
      Pyramide de profiteroles, glace spéculoos, sauce chocolat blanc.<br>
    </p>
    <p>
      <u>
      et verrines sucrées (2 choix possible):<br>
      </u>
      verrine de confit ananas, crème de mascarpone à la vanille et broyé de spéculoos.<br>
      verrine mousse chocolat noir, craquant chocolat blanc et griottes.<br>
      verrine mousse chocolat blanc, craquant chocolat noir et griottes.<br>
      verrine de minestrone de fruits au sirop épicé.<br>
      verrine mousse de fromage blanc, coulis de fruits rouges, groseilles.<br>
      verrine deTiramisu.<br>
      verrine de chti'ramisu (genièvre et spéculoos).<br>
      verrine de mousseline de pain d'épices et pommes confites au miel.<br>
      verrine de panacotta au citron vert et à la noix de coco, coulis de mangue.<br>
    </p>
    <p class="fin">
      Ce barbecue party sera réalisé dans votre cuisine, soigneusement nettoyée et rangée par mes soins après le service.<br>
      Un forfait de déplacement sera demandé pour une distance supérieur à 30 kms de mon domicile.
    </p>
  </div>
</template>
